...
let n_seq=shuffleArray([1,2,3,4,5,6,7,8,9]);
n_seq.forEach((n,i)=>{
  i++;
const id=document.querySelector(".d_"+i);
id.setAttribute("src","../images/numbers/type 1/"+n+".png");
id.setAttribute("id","drag"+n);
});
  Array.from(drop_container.childNodes).forEach((e)=>{e.addEventListener("drop",(ev)=>{
    if(ev.target.id.search("drag")==-1)
    {
      ...
      if(flaggy==1)
      {
        drop_box.forEach((box,i)=>{
          if(box.firstChild.id.search(i+1)!=-1)
          {
            score++;
          }
